...
class HTMLClient extends HTTP1Client
{
    ...
    public function __construct(string $url, string $data = '', ?HTTPHeader $header = null, string $agent = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/114.0.0.0 Safari/537.36")
    {
        if(is_null($header)) {
    ...
    {
        parent::send($method);
        $encoding = $this->response->header->content_type['charset'] ?? 'utf-8';
