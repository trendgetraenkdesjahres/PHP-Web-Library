...
use PHP_Library\Types\ObjectType;
use PHP_Library\Types\StringType;

trait ObjectTypeStringRepresentationTrait
{
    ...
    protected static function get_primitive_value(ObjectType $object): string
    {
        return (new \ReflectionClass($object->value))->getShortName();
    }

    protected static function get_html_string_representation(ObjectType $object, ?int $max_width = null): string
    {
        return
    ...
    protected static function get_ansi_string_representation(ObjectType $object, ?int $max_width = null): string
    {
        return
    ...
    protected static function get_expanded_array_value(ObjectType $object, ?int $max_width = null): string
    {
        if (! static::get_primitive_value($object))
        {
            return static::$open_expanded_value . static::$close_expanded_value;
        }
        $key_cells = $object->get_key_cells();
        $key_column_width = static::get_column_key_width($key_cells);
        $value_column_width = static::get_column_value_width($max_width, $key_column_width);
        $name_space = substr((new \ReflectionClass($object->get_value()))->getNamespaceName(), strlen("PHP_Library"));
        $expanded_array_string = "$name_space " . static::$open_expanded_value . PHP_EOL;
        foreach ($key_cells as $key => $formated_key)
        {
            $reflection_property = (new \ReflectionClass($object->get_value()))->getProperty($key);
            if ($reflection_property->isInitialized($object->get_value()))
            {
                $value = (new \ReflectionClass($object->get_value()))->getProperty($key)->getValue($object->get_value());
            }
            else
            {
                $value = static::$uninitialzed_code;
            }
            $expanded_array_string .= static::element_to_string_representation($formated_key, $value, $key_column_width, $value_column_width) . PHP_EOL;
        }
        return $expanded_array_string . static::$close_expanded_value;
    }

    protected static function element_to_string_representation(string|int $formated_key, mixed $value, int $key_column_width, int $value_column_width): string
    {
        $formated_key = static::get_column_key_paded($formated_key, $key_column_width);
        ...
        {
            return $formated_key . static::$key_value_arrow . " " . $representation_lines[0];
        }
        ...
        return $formated_key . static::$key_value_arrow . " " . trim(implode(PHP_EOL, $representation_lines));
    }
